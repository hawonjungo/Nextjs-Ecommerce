        ...
        <div className="h-[calc(100vh-80px)] overflow-hidden">
                ...
                {slides.map(slide => (
                    <div key={slide.id} className={`${slide.bg}  w-screen h-full flex flex-col gap-16 xl:flex-row `}>
                        ...
                        <div className="h-1/2 xl:w-1/2 xl:h-full flex flex-col items-center justify-center text-center gap-8 2xl:gap-12">
                            ...
                            <Link href={slide.url}>
                                <button className="bg-black text-white rounded-md py-3 px-4">SHOP NOW</button>
                            </Link>
                        </div>
                        {/* IMAGE */}
            ...
            <div className="absolute m-auto left-1/2 bottom-8 flex gap-4">
                {slides.map((slide,index)=>(
                    <div key={slide.id} onClick={()=>setCurrent(index)} className={`${current ===index ? "scale-150" : ""} w-3 h-3 ring-1 ring-gray-600 rounded-md cursor-pointer flex justify-center items-center`}>
                        <div className="w-2 h-2 bg-gray-600 rounded-md"></div>
                    </div>
                ))}
            </div>
        </div>
